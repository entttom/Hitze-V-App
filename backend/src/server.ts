...
import {
  getPushoverReportStatus,
  isPushoverConfigured,
  setPushoverReportsEnabled,
} from "../api/cron/pushover";
import {
...
let cachedPoliticalDistrictFeatures: TestMapFeature[] | null = null;
let cachedMunicipalityFeatures: TestMapFeature[] | null = null;

interface TestMapFeature {
  id: string;
  name: string;
  geometry: unknown;
}
...
function mapGeoJsonPaths(fileName: string): string[] {
  return [
...
function loadMapFeatures(fileName: string, cacheName: "districts" | "municipalities"): TestMapFeature[] {
  ...
  for (const filePath of mapGeoJsonPaths(fileName)) {
    ...
    try {
      ...
      const parsedFeatures: TestMapFeature[] = [];
      for (const feature of features) {
        ...
        const record = feature as {
          id?: unknown;
          properties?: { id?: unknown; districtId?: unknown; name?: unknown };
          geometry?: unknown;
        };
...
function loadPoliticalDistrictFeatures(): TestMapFeature[] {
  return loadMapFeatures(POLITICAL_DISTRICTS_GEOJSON_FILE, "districts");
}

function loadMunicipalityFeatures(): TestMapFeature[] {
...
function renderTopNav(activeHref: string): string {
...
function renderIndexPage(developMode: boolean): string {
...
function renderPushoverSettingsPage(): string {
  return `<!doctype html>
<html lang="de">
  <head>
    <meta charset="utf-8" />
...
if (developMode) {
  ...
  app.get("/test/pushover/ui", (_req: Request, res: Response) => {
    res.setHeader("Cache-Control", "no-store, no-cache, must-revalidate, proxy-revalidate");
    res.setHeader("Pragma", "no-cache");
    res.setHeader("Expires", "0");
    res.setHeader("Surrogate-Control", "no-store");
  ...
  app.get("/test/pushover/status", async (_req: Request, res: Response) => {
    res.setHeader("Cache-Control", "no-store, no-cache, must-revalidate, proxy-revalidate");
    res.setHeader("Pragma", "no-cache");
    res.setHeader("Expires", "0");
    res.setHeader("Surrogate-Control", "no-store");
    ...
    try {
      res.status(200).json(await getPushoverReportStatus());
    } catch (error) {
      res.status(500).json({
  ...
  app.post("/test/pushover/status", async (req: Request, res: Response) => {
    ...
    try {
      res.status(200).json(await setPushoverReportsEnabled(enabled));
    } catch (error) {
      res.status(500).json({
